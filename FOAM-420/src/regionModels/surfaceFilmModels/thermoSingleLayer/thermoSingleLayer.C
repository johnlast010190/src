...
void thermoSingleLayer::resetPrimaryRegionSourceTerms()
{
    ...
    hsSpPrimary_.forceAssign(dimensionedScalar("zero", hsSp_.dimensions(), 0.0));
}


void thermoSingleLayer::correctThermoFields()
{
    rho_.forceAssign(filmThermo_->rho());
    sigma_.forceAssign(filmThermo_->sigma());
    Cp_.forceAssign(filmThermo_->Cp());
    kappa_.forceAssign(filmThermo_->kappa());
}
...
//    }
    ...
    forAll(T_.boundaryField(), pI)
    {
        const fvPatchField<scalar>& Tp = T_.boundaryField()[pI];
        const polyPatch& pp = regionMesh().boundaryMesh()[pI];
        if (isA<mappedFieldFvPatchField<scalar>>(Tp))
        {
            scalarField nbrIntFld;
            const mappedFieldFvPatchField<scalar>& cP =
                refCast<const mappedFieldFvPatchField<scalar>>(Tp);

            const label samplePatchI = cP.samplePolyPatch().index();
            const fvPatchField<scalar>& nbrPatchField =
                cP.sampleField().boundaryField()[samplePatchI];
            nbrIntFld = nbrPatchField.patchInternalField();
            cP.distribute(nbrIntFld);
            UIndirectList<scalar>(Tw_.internalField(), pp.faceCells())
                = nbrIntFld;
        }
    }
    ...
    Tmax_(VGREAT)
{
    ...
    if (readFields)
    {
        ...
        hs_.forceAssign(hs(T_));

        deltaRho_.forceAssign(delta_*rho_);
...
void thermoSingleLayer::preEvolveRegion()
{
    ...
    // Update phase change
    primaryMassPCTrans_.forceAssign(dimensionedScalar("zero", dimMass, 0.0));
    primaryEnergyPCTrans_.forceAssign(dimensionedScalar("zero", dimEnergy, 0.0));
}
