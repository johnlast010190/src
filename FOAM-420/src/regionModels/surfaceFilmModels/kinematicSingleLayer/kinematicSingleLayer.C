...
void kinematicSingleLayer::correctThermoFields()
{
    ...
    mu_.forceAssign(filmThermo_->mu());
...
void kinematicSingleLayer::resetPrimaryRegionSourceTerms()
{
    ...
    rhoSpPrimary_.forceAssign(dimensionedScalar("zero", rhoSp_.dimensions(), 0.0));
    USpPrimary_.forceAssign(dimensionedVector("zero", USp_.dimensions(), Zero));
    pSpPrimary_.forceAssign(dimensionedScalar("zero", pSp_.dimensions(), 0.0));
}
...
{
    alpha_.forceAssign(pos0(delta_ - deltaSmall_));
}
...
)
{
   ...
   // - fvm::SuSp(rhoSp_, U_)
    ...
    if (momentumPredictor_)
    {
        ...
        // correct velocities in fvOptions
...
)
{
    ...
    for (int nonOrth=0; nonOrth<=nNonOrthCorr_; nonOrth++)
    {
        ...
        if (nonOrth == nNonOrthCorr_)
        {
            ...
            phi_.forceAssign(deltaEqn.flux());
        }
    }
...
void kinematicSingleLayer::preEvolveRegion()
{
    ...
    cloudMassTrans_.forceAssign(dimensionedScalar("zero", dimMass, 0.0));
    cloudDiameterTrans_.forceAssign(dimensionedScalar("zero", dimLength, 0.0));
}
