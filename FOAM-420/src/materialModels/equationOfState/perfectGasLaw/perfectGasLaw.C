...
void Foam::perfectGasLaw::updateTable(const word& modelName)
{
    ...
    forAll(modList, i)
    {
        fill(modelName, modList[i], {RName}, {R}, {true}, i);
        R_ = (modelName == modList[i]) ? value(RName, R) : R_;
    }
}
...
bool Foam::perfectGasLaw::read()
{
    ...
    // For re-reading the dicts
    if (sMod_(R) != nullptr)
    {
        R_ = value(RModel::typeName, R, 0, true, true);
    }
