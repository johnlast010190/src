...
Foam::baseModels<Foam::scalar>* Foam::incompressiblePerfectGasLaw::castScalarModel
(
...
bool Foam::incompressiblePerfectGasLaw::read()
{
    ...
    const refScalarField& p = *constructOrReturnRefFieldPtr<scalar>("p");
    if (dict_->found("pRef"))
    {
        DeprecationIOWarningInFunction(dict_, "pRef", "entry", 40200)
            << "Please specify gauge pressure with 'pConst' instead" << endl;
        pRef_ = dict_->lookup<scalar>("pRef");
    }
    else if (p.isConst() && !dict_->found("pConst"))
    {
        pRef_ = p.constant().value() + p.offset().value();
    }
    else
    {
        pRef_ = dict_->lookup<scalar>("pConst") + p.offset().value();
    }
    if (sMod_(R) != nullptr)
    {
        sMod_[R].read();
