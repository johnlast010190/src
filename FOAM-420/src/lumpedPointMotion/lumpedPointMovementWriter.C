...
) const
{
    ...
    for (const label patchId : patchIds)
    {
        ...
        if (!isA<indirectPolyPatch>(pp))
        {
            ...
            format()
                .openTag(vtk::fileTag::PIECE)
                .xmlAttr(vtk::fileAttr::NUMBER_OF_POINTS, dpp.nPoints())
                .xmlAttr(vtk::fileAttr::NUMBER_OF_POLYS,  dpp.size())
                .closeTag();

            // 'points'
            {
                const uint64_t payLoad = (dpp.nPoints()*3* sizeof(float));
                ...
                (
                    ...
                    points0,
                    dpp.meshPoints()
                ) + pointField(points0, dpp.meshPoints());
            ...
            //
            {
                ...
                for (const face& f : dpp.localFaces())
                {
                    vtk::writeList(format(), f);
                }
                format().flush();
            ...
            //
            {
                const uint64_t payLoad = (dpp.size() * sizeof(label));
