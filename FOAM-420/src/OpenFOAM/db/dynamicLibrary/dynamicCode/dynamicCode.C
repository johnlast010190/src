...
const char* const Foam::dynamicCode::libTargetRoot =
    "LIB = $(PWD)/../platforms/$(FOAM_OPTIONS)/lib/lib";
...
)
{
    ...
    if (!allowSystemOperations)
    {
        FatalIOErrorInFunction(dict)
            ...
            << "or" << nl << nl
            << "    $FOAM_PROJECT_DIR/etc/controlDict" << nl
            << endl
            << exit(FatalIOError);
    }
}
...
{
    // Try to get template from FOAM_CODESIREAM_TEMPLATES
...
bool Foam::dynamicCode::createMakeFile() const
{
    ...
    string foamOptions = stringOps::expand("$FOAM_OPTIONS", true);
    if (string::null == foamOptions)
    {
        WarningInFunction
            << "The environment variable FOAM_OPTIONS was not set."
    ...
    fileName makefileTemplate =
        findEtcFile
        (
            fileName("codeTemplates/dynamicCode/makefiles") / foamOptions /
...
:
    ...
    libSubDir_(stringOps::expand("platforms/$FOAM_OPTIONS/lib")),
    codeName_(codeName),
    codeDirName_(codeDirName)
{
    ...
    List<string> requiredEnvVars{"$FOAM_OPTIONS", "$FOAM_PROJECT_DIR"};
