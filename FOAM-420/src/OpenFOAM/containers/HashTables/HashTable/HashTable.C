...
)
{
    ...
    if (!existing)
    {
        ...
        table_[hashIdx] = new hashedEntry(key, std::move(obj), table_[hashIdx]);
        nElmts_++;
    ...
    else
    {
        ...
        hashedEntry* ep = new hashedEntry(key, std::move(obj), existing->next_);
...
void Foam::HashTable<T, Key, Hash>::resize(const label sz)
{
    ...
    HashTable<T, Key, Hash> tmpTable(newSize);

    for (iterator iter = begin(); iter != end(); ++iter)
    {
        tmpTable.emplace(iter.key(), std::move(iter.object()));
    }
