...
) const
{
    ...
    (
        static_cast<const fvMesh&>(mesh).objectRegistry::lookupClass<fldType>()
    );
...
)
{
    ...
    forAllIter(typename HashTable<GeoField*>, flds, iter)
    {
        ...
        forAll(bfld, patchi)
        {
            ...
            {
                bfld[patchi].forceAssign(initVal);
            }
        }
    }
}
...
void Foam::fvMeshDistribute::resetUpdate()
{
    ...
    forAllIter(typename HashTable<GeoField*>, flds, iter)
    {
        ...
        forAll(fld.boundaryField(), patchi)
        {
            ...
            {
                fld.boundaryFieldRef()[patchi].resetUpdate();
            }
        }
    }
}
...
)
{
    ...
    forAll(fieldNames, i)
    {
        ...
        tmp<GeoField> tsubfld = subsetter.interpolate(fld, /*handleInternal*/true);
...
)
{
    HashTable<GeoField*> flds
    (
        baseMesh.objectRegistry::lookupClass<GeoField>()
    );

    fields.setSize(flds.size());
    ...
    forAllIter(typename HashTable<GeoField*>, flds, iter)
    {
        ...
        if (subsetterPtr.valid())
        {
            ...
            tmp<GeoField> tsubfld = subsetterPtr().interpolate(fld);
