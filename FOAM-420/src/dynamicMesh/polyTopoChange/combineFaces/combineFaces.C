...
) const
{
    ...
    forAll(cEdges, i)
    {
        ...
        if
        (
            (p0 != -1 && p1 !=-1 && (!samePatch_ || (p0 == p1)) && !patches[p0].coupled()
             && !patches[p1].coupled() && !fixedEdges_[edgeI])
        )
        {
            ...
            if (surfNormalsPtr_)
            {
                ...
                {
                    f0Normal = sn[f0];
                    f0Normal /= mag(f0Normal) + SMALL;
                    f1Normal = sn[f1];
                    f1Normal /= mag(f1Normal) + SMALL;
                }
                else
                {
                    foundEdge = false;
    ...
    surfNormalsPtr_(surfNormals)
{
    ...
    fixedEdges_.setSize(mesh_.edges().size());
    forAll(fixedEdges_,j)
    {
        fixedEdges_[j] = false;
    }

    //loop through all the featureShadows finding mesh edges that need to be preserved
