...
)
{
    ...
    if (handleInternal)
    {
        ...
        GeometricField<Type, fvsPatchField, surfaceMesh> vfs( fvc::interpolate(vf,schemeData) );
        ...
        forAll(bf, patchi)
        {
            if (patchMap[patchi] == -1)
            {
                ...
                forAll(pfld, i)
                {
                    ...
                    {
                        label mappedPatch = vf.mesh().boundaryMesh().whichPatch(oldFacei);
                        label index = oldFacei - vf.mesh().boundary()[mappedPatch].start();

                        pfld[i] = vf.boundaryField()[mappedPatch][index];
                    }
                }
            }
        }
