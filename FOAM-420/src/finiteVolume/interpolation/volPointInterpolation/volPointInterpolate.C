...
) const
{
    ...
    if (!exclude())
    {
        ...
        forAll(pfbf, patchi)
        {
            word patchType(pfbf[patchi].type());
            if
            (
                pfbf[patchi].coupled()
                &&
                (
                    patchType != cyclicAMIPointPatch::typeName
                    && patchType != cyclicACMIPointPatch::typeName
                )
            )
            {
...
) const
{
    ...
    {
        if (isA<indirectPolyPatch>(pbm[patchi]))
        {
            continue;
        }
        label bFacei = bm[patchi].patch().start() - mesh.nInternalFaces();
        word patchType(bm[patchi].type());
        if
        (
           ...
           (
               ...
               || patchType == cyclicAMIPolyPatch::typeName
               || patchType == cyclicACMIPolyPatch::typeName
           )
        )
        {
            SubList<Type>
