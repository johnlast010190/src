...
void Foam::volPointInterpolation::calcBoundaryAddressing()
{
    ...
    //parallel synchronise patch point ids
...
void Foam::volPointInterpolation::makeInternalWeights(scalarField& sumWeights)
{
    ...
    if (pLweights_)
    {
        // Calculate pseudo-Laplacian weighting between cell centres
        // and points and store in weighting factor array
        if (debug)
        {
            Info<< "Using pseudo-Laplacian weighting "
                 << "for volPointInterpolation." << endl;
        }

        tensorField I(points.size(), tensor::zero);
        vectorField R(points.size(), vector::zero);
        ...
        forAll(points, pointi)
        {
            if (!isPatchPoint_[pointi])
            {
                ...
                forAll(pcp, pointCelli)
                {
                    ...
                    R[pointi] += delta;
                    I[pointi] += (delta*delta);
                }

            }
        }

        //do pseudoLaplacian boundary interp here so we only have to sync once
        const primitivePatch& boundary = boundaryPtr_();
        //const vectorField& pointNormals = boundary.pointNormals();
        ...
        forAll(boundary.meshPoints(), i)
        {
            ...
            if (isPatchPoint_[pointI])//do here so we only have to sync once
            {
                ...
                forAll(pFaces, j)
                {
                    ...
                    {
                        label faceI = mesh().nInternalFaces() + pFaces[j];
                        vector delta
        ...
        forAll(boundary.meshPoints(), i)
        {
            ...
            if (isPatchPoint_[pointI])//do here so we only have to sync once
            {
                //We stabilise the inversion of the surface I tensor
                //by introducing a slight surface normal displacement of
                // the point
                vector L(hinv(I[pointI]) & R[pointI]);
                const labelList& pFaces = boundary.pointFaces()[i];
    ...
    else
    {
        ...
        forAll(points, pointi)
        {
            if (!isPatchPoint_[pointi])
            {
                ...
                forAll(pcp, pointCelli)
                {
                    scalar delta = max(mag(points[pointi] - cellCentres[pcp[pointCelli]]), SMALL);

                    pw[pointCelli] = 1.0/delta;
            ...
            else
            {
                ...
                forAll(pFaces, i)
                {
                    if (boundaryIsPatchFace_[pFaces[i]])
                    {
                        ...
                        scalar delta = max(mag(points[pointi] - faceCentres[facei]), SMALL);
...
bool Foam::volPointInterpolation::volPointInterpolation::exclude() const
{
    if (!Pstream::parRun() && Pstream::nProcs() != 0)
    {
        return true;
    }
...
:
    ...
    pLweights_(false)
{
    if (vm.schemes().found("interpolationSchemes"))
    {
        pLweights_ =
            vm.schemes().subDict("interpolationSchemes")
            .lookupOrDefault<Switch>
            ("pseudoLaplacianVolPointInterpolation", false);
    }
