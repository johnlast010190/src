...
)
{
    ...
    forAll(allOrigPatchIDs, i)
    {
        ...
        forAll(origPp, origPatchFacei)
        {
            ...
            const part smallP
            (
                patchToPatch::small*origPp.faceAreas()[origPatchFacei],
...
)
{
    ...
    // values stored on the original faces. The original field overwrites the
    // existing boundary values, whilst the cyclic field is stored as a
    // separate field for later use.
...
)
{
    ...
    if (load)
    {
        ...
        if (fileHandler().isFile(polyFacesBfIO.objectPath()))
        {
            ...
            // Read the boundary field but then set default values for
            // conformal patches as these patches will have had a uniform
            // invalid index set in order to save disk space
    ...
    // parts. The original part was stored in the same field, whilst the cyclic
