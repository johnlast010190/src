...
)
{
    ...
    if (dict.found("transforms"))
    {
        ...
        forAll(transforms, dictI)
        {
            ...
            else if (type == "recess")
            {
                ...
                v /= mag(v);
                scalar t =
                    transformDict.lookupOrDefault("distance",scalar(0.0));
                bool project = transformDict.lookupOrDefault<bool>
                (
                    "projectToPlane",
                    false
                );
                ...
                vectorField zoneCentroids(numZones, vector::zero);
                vectorField zoneNormals(numZones, vector::zero);
    ...
    {
        Info<< name << " : using scale " << scaleFactor
            << endl;
    ...
    points0Ptr_(nullptr)
{
    ...
    if (io.time().foundObject<IOdictionary>(geomDict))
    {
        ...
        if (globalGeomDict.found("surfaces"))
        {
            ...
            if (sdict.found(name))
            {
                const dictionary& tdict = sdict.subDict(name);
    ...
    if (io.time().foundObject<IOdictionary>(geomDict))
    {
        ...
        word frameName = dict.lookupOrDefault<word>("referenceFrame", "");
        dictionary frameDict = dictionary();
        if (dict.found(frameName))
        {
            frameDict = dict.lookup("frameName");
        }

        //Add local/global transform support
        if (globalGeomDict.found("surfaces"))
        {
             const dictionary& sdict = globalGeomDict.subDict("surfaces");
             if (sdict.found(name))
             {
                //Add reference frame support
                 if (sdict.found("referenceFrame"))
                 {
                     frameName = sdict.lookup<word>("referenceFrame");
                     if (sdict.found(frameName))
                     {
                         frameDict = dict.lookup("frameName");
                     }
                 }
        ...
        {
            autoPtr<coordinateSystem> coordinateSystemTime0;
            coordinateSystemTime0.reset
            (
               frameData::frameToGlobal(frameDict, io.time())
            );
            movePoints
            (
               coordinateSystemTime0().localPosition(triSurface::points())
            );
        }
    }
