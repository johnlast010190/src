...
{
    //Info<< "In calcMapping " << patch_.name() << endl;
    ...
    // NEARESICELL since finds cell containing a location
    if (mode_ == NEARESICELL)
    {
        label nNotFound = 0;
...
:
    ...
    mapPtr_(nullptr),
    AMIPtr_(nullptr),
    AMIInterpolationMethod_(AMIPatchToPatchInterpolation::imFaceAreaWeight),
...
:
    ...
    mode_
    (
        sampleModeNames_.lookupOrDefault("sampleMode", dict, NEARESIPATCHFACE)
    ),
    ...
    surfDict_(dict.subOrEmptyDict("surface"))
{
    ...
    else if (mode_ != NEARESIPATCHFACE && mode_ != NEARESIPATCHFACEAMI)
    {
        FatalIOErrorInFunction
        (
            dict
...
:
    ...
    surfDict_(dict.subOrEmptyDict("surface"))
{
    if (mode != NEARESIPATCHFACE && mode != NEARESIPATCHFACEAMI)
    {
        FatalIOErrorInFunction
        (
            dict
            ...
            << sampleModeNames_[NEARESIPATCHFACE] << ','
            << sampleModeNames_[NEARESIPATCHFACEAMI]
            << exit(FatalIOError);
    }
...
void Foam::mappedPatchBase::write(Ostream& os) const
{
    ...
    (
        offsetMode_ == UNIFORM
     && offset_ == vector::zero
     && (mode_ == NEARESIPATCHFACE || mode_ == NEARESIPATCHFACEAMI)
    )
    {
