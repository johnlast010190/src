...
)
{
    ...
    // 1-D plan
    // fftw_plan plan =
    //    fftw_plan_dft_1d(N, in, out, FFTW_FORWARD, FFTW_ESIIMATE);

    // Generic 1..3-D plan
    label rank = nn.size();
    fftw_plan plan =
        fftw_plan_dft(rank, nn.begin(), in, out, dir, FFTW_ESIIMATE);
...
/*
    ...
    fftw_plan plan = fftw_plan_r2r_2d(N, in, out, FFTW_R2HC, FFTW_ESIIMATE)
