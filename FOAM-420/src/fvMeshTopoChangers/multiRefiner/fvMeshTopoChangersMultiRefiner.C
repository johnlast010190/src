...
Foam::scalarField
Foam::fvMeshTopoChangers::multiRefiner::minCellField(const volScalarField& vFld) const
{
...
void Foam::fvMeshTopoChangers::multiRefiner::updateRefinementField()
{
    ...
    else
    {
        ...
        forAll(fieldNames, i)
        {
            ...
            const volScalarField& fld = mesh().lookupObject<volScalarField>(fldName);
        ...
        forAll(curlFieldNames, i)
        {
            ...
            const volVectorField& fld = mesh().lookupObject<volVectorField>(fldName);
        ...
        forAll(interfaceRefineField, i)
        {
            ...
            surfaceScalarField deltaAlpha ( mag(fvc::snGrad(fld) / mesh().deltaCoeffs()) );
...
bool Foam::fvMeshTopoChangers::multiRefiner::update()
{
    ...
    if (mesh().time().value() >= refineStartTime_ && mesh().time().value() <= refineStopTime_)
    {
        if (enableRefinementControl_ && mesh().time().timeIndex() > 0
