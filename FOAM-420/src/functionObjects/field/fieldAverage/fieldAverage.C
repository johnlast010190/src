...
void Foam::functionObjects::fieldAverage::resetFields()
{
    forAll(faItems_, i)
    {
        ...
        if (faItems_[i].minFlag())
        {
            if (obr().found(faItems_[i].minFieldName()))
            {
                obr().checkOut(*obr()[faItems_[i].minFieldName()]);
            }
        }

        if (faItems_[i].maxFlag())
        {
            if (obr().found(faItems_[i].maxFieldName()))
            {
                obr().checkOut(*obr()[faItems_[i].maxFieldName()]);
            }
        }
    }
}
...
void Foam::functionObjects::fieldAverage::initialize()
{
    ...
    // Add min fields to the field lists
    forAll(faItems_, fieldi)
    {
    ...
    // Add max fields to the field lists
    forAll(faItems_, fieldi)
    {
...
)
{
    ...
    else
    {
        ...
        forAll(faItems_, fieldi)
        {
            ...
            {
                scalar tStart  = obr().time().timeOutputValue();
                dict.readIfPresent("timeStart", tStart);
                Info<< "        " << fieldName
