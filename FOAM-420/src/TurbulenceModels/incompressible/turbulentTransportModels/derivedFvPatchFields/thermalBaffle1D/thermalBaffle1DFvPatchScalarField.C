...
:
    ...
    (
      ...
      ? static_cast<const directFvPatchFieldMapper&>(mapper).addressing()
      : static_cast<const generalFvPatchFieldMapper&>(mapper)
       .directAddressing()
    ),
...
void thermalBaffle1DFvPatchScalarField<solidType>::updateCoeffs()
{
    ...
    if (baffleActivated_)
    {
        ...
        const incompressibleTurbulenceModel& turbModel =
            db().template lookupObject<incompressibleTurbulenceModel>
            (
                IOobject::groupName
                (
        ...
        const scalarField kappaw
        (
            turbModel.alphaEff()().boundaryField()[patchi]
            *turbModel.Cp()().boundaryField()[patchi]
        ...
        const scalarField nbrKappaw
        (
            turbModel.alphaEff()().boundaryField()[nbrPatchi]
            *turbModel.Cp()().boundaryField()[nbrPatchi]
        );
