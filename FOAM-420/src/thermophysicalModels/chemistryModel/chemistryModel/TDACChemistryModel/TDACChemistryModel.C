...
:
    chemistryModel<CompType, ThermoType>(obr, phaseName),
    variableTimeStep_
    (
        obr.time().controlDict().lookupOrDefault("adjustTimeStep", false)
...
{
    speciesMassFractions& composition = this->thermo().composition();
...
) const
{
    ...
    {
        const scalar Told = this->T_[0];
        const scalar pold = this->p_[0];
        this->T_[0] = T;
        this->p_[0] = p;
        for (label i=0; i<this->c_.size(); i++)
        {
            rho += this->WModels_[i][0]*this->c_[i];
        }
        for (label i=0; i<this->c_.size(); i++)
        {
            cp += this->c_[i]*this->CpModels_[i][0]*this->WModels_[i][0];
        }
        cp /= rho;
        ...
        {
            const label si =
                reduced ? simplifiedToCompleteIndex_[i] : i;
            // ha function returns [J/kmol]
            const scalar hi =
                this->HaModels_[si][0]*this->WModels_[si][0];
            dT += hi*dcdt[i];
        }
        this->T_[0] = Told;
        this->p_[0] = pold;
    ...
    else
    {
        for (label i=0; i<this->c_.size(); i++)
        {
            rho += this->specieThermo_[i].W()*this->c_[i];
        }
        for (label i=0; i<this->c_.size(); i++)
        {
        ...
        for (label i=0; i<this->nSpecie_; i++)
        {
            const label si =
                reduced ? simplifiedToCompleteIndex_[i] : i;
            // ha function returns [J/kmol]
            scalar hi = this->specieThermo_[si].ha(p, T);
            dT += hi*dcdt[i];
        }
    }
    dT /= rho*cp;
...
)
{
    ...
    forAll(rho, celli)
    {
        ...
        scalar pi = this->p_[celli];
        scalar Ti = this->T_[celli];

        for (label i=0; i<this->nSpecie_; i++)
        {
            c[i] = rhoi*this->Y_[i][celli]/(this->isMatModels_ ? this->WModels_[i][0] : this->specieThermo_[i].W());
            c0[i] = c[i];
        ...
        if (tabulation_->active() && tabulation_->retrieve(phiq, Rphiq))
        {
            ...
            for (label i=0; i<this->nSpecie(); i++)
            {
                c[i] =
                    rhoi*Rphiq[i]
                   /(this->isMatModels_ ? this->WModels_[i][0] : this->specieThermo_[i].W());
            }
        ...
        else
        {
            ...
            while (timeLeft > SMALL)
            {
                ...
                if (reduced)
                {
                    ...
                    this->solve
                    (
                        simplifiedC_, Ti, pi, dt, this->deltaTChem_[celli], celli
                    );
                ...
                {
                    this->solve(c, Ti, pi, dt, this->deltaTChem_[celli], celli);
                }
                timeLeft -= dt;
            }
            ...
            if (tabulation_->active())
            {
                forAll(c, i)
                {
                    Rphiq[i] = c[i]/rhoi*(this->isMatModels_ ? this->WModels_[i][0] : this->specieThermo_[i].W());
                }
                if (tabulation_->variableTimeStep())
                {
        ...
        {
            this->RR_[i][celli] =
                (c[i] - c0[i])*(this->isMatModels_ ? this->WModels_[i][0] : this->specieThermo_[i].W())/deltaT[celli];
        }
    }
