...
)
{
    ...
    const auto ctor = ctorTableLookup("absorptionEmissionModel type", dictionaryConstructorTable_(), modelType);
    return autoPtr<absorptionEmissionModel>(ctor(dict, mesh));
}
