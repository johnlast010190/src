...
template<class ReactionThermo>
void Foam::Reaction<ReactionThermo>::initMaterialModels() const
{
    const objectRegistry& obr = this->mesh().thisDb();
    const objectRegistry& matObr = obr.subRegistry("materialModels");
    materialTables& mat = matObr.lookupObjectRef<materialTables>("materialTables");
    T_ = obr.lookupObjectRefPtr<scalarField>("T");
    p_ = obr.lookupObjectRefPtr<scalarField>("p");
    coeffs_ = 0.0;
    word specieName = word::null;
    lhsGModels_.resize(lhs_.size());
    lhsWs_ = scalarField(lhs_.size(), 0.0);
    forAll(lhs_, i)
    {
        specieName = species_[lhs_[i].index];
        lhsGModels_.set
        (
            i,
            mat.sTable(phaseName_, specieName)[GModel::typeName]
        );
        lhsWs_[i] = mat(WModel::typeName, phaseName_, specieName)[0];
        coeffs_ -= lhs_[i].stoichCoeff;
    }

    rhsGModels_.resize(rhs_.size());
    rhsWs_ = scalarField(rhs_.size(), 0.0);
    forAll(rhs_, i)
    {
        specieName = species_[rhs_[i].index];
        rhsGModels_.set
        (
            i,
            mat.sTable(phaseName_, specieName)[GModel::typeName]
        );
        rhsWs_[i] = mat(WModel::typeName, phaseName_, specieName)[0];
        coeffs_ += rhs_[i].stoichCoeff;
    }
    isMatInit_ = true;
}
...
{
    if (isType<dummyThermo>(typename ReactionThermo::thermoType(r)))
    {
        isMaterials_ = true;
    }
}
...
)
{
    ...
    const auto ctor = ctorTableLookup("reaction type", dictionaryConstructorTable_(), reactionTypeName);
    return autoPtr<Reaction<ReactionThermo>>
    (
        ctor(species, thermoDatabase, dict)
    );
}
...
template<class ReactionThermo>
Foam::scalar Foam::Reaction<ReactionThermo>::Kc
(
    const scalar p,
    const scalar T
) const
{
    if (isMaterials_)
    {
        ...
        const scalar Told = T_->operator[](0);
        const scalar pOld = p_->operator[](0);
        T_->operator[](0) = T;
        p_->operator[](0) = Pstd;
        scalar G = 0.0;
        forAll(lhs_, i)
        {
            G -= lhsGModels_[i][0]*lhsWs_[i]*lhs_[i].stoichCoeff;
        }
        forAll(rhs_, i)
        {
            G += rhsGModels_[i][0]*rhsWs_[i]*rhs_[i].stoichCoeff;
        }
        T_->operator[](0) = Told;
        p_->operator[](0) = pOld;
        const scalar arg = -G/(RR*T);
        const scalar Kp = (arg < 600) ? exp(arg) : VGREAT;
        return equal(coeffs_, SMALL) ? Kp : Kp*pow(Pstd/(RR*T), coeffs_);
    }

    return ReactionThermo::thermoType::Kc(p, T);
}
