...
)
{
    ...
    const auto ctor = ctorTableLookup("chemistryReader type", dictionaryConstructorTable_(), chemistryReaderTypeName);
    return autoPtr<chemistryReader<ThermoType>>
    (
        ctor(thermoDict, species, obr)
    );
}

template<class ThermoType>
const Foam::fvMesh& Foam::chemistryReader<ThermoType>::getMesh(const objectRegistry& obr) const
{
