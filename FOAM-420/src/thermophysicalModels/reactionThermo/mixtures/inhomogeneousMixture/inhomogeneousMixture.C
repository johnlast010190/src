...
:
    ...
    fuel_(mesh, thermoDict.subDict("fuel")),
    oxidant_(mesh, thermoDict.subDict("oxidant")),
    products_(mesh, thermoDict.subDict("burntProducts")),
...
template<class ThermoType>
void Foam::inhomogeneousMixture<ThermoType>::read(const objectRegistry& mesh, const dictionary& thermoDict)
{
    thermoDict.lookup("stoichiometricAirFuelMassRatio") >> stoicRatio_;

    fuel_ = ThermoType(mesh, thermoDict.subDict("fuel"));
    oxidant_ = ThermoType(mesh, thermoDict.subDict("oxidant"));
    products_ = ThermoType(mesh, thermoDict.subDict("burntProducts"));
}
