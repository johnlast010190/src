    ...
    forAll(pBf, patchi)
    {
        ...
        if (pT.fixesValue())
        {
            forAll(pT, facei)
            {
                ...
                prho[facei] = mixture_.rho(pp[facei] + this->pRefValue(), pT[facei]);
                pmu[facei] = mixture_.mu(pp[facei] + this->pRefValue(), pT[facei]);
                palpha[facei] = mixture_.alphah(pp[facei] + this->pRefValue(), pT[facei]);
            }
        }
        else
        {
...
:
    heThermo<BasicPsiThermo, MixtureType>(obr, phaseName)
{
    calculate
    (
        this->p_,
