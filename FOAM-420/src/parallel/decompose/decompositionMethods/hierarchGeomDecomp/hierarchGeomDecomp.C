...
)
{
    ...
    while (true)
    {
        ...
        reduce(std::tie(size, hasNotChanged), ParallelOp<sumOp<label>, andOp<bool>>{});
...
)
{
    ...
    while (true)
    {
        scalar weightedSize = sortedWeightedSizes[mid] - sortedWeightedSizes[minIndex];
        reduce(std::tie(weightedSize, hasNotChanged), ParallelOp<sumOp<scalar>, andOp<bool>>{});
