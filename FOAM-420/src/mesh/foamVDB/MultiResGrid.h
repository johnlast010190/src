...
public:
    ...
    /// @brief Given an initial low-resolution grid this constructor
    /// generates all the finer grids by means of prolongation.
    /// @param levels The number of trees in this MultiResGrid
    /// @param coarseGrid Low-resolution input grid
    /// @param voxelSize Size of a (uniform voxel) of finest tree.
    /// @note This constructor will steal the input grid and use it
    /// as the lowest level grid. On output the coarseGrid is empty.
    MultiResGrid(size_t levels, GridPtr coarseGrid, double voxelSize);
    ...
    // Public struct that concurrently creates a mask of active voxel
    // in a coarse (fine) tree from the active voxels in a fine (coarse) tree
    struct MaskOp;
    struct MaskFineTreeOp;
...
{
    this->initMeta();
    mTrees[this->coarsestLevel()] = coarseGrid->treePtr();// steal tree from input grid
    mTrees[this->coarsestLevel()]->voxelizeActiveTiles();
    coarseGrid->newTree();
    this->bottomUpProlongate();
}
...
grid(size_t level)
{
    ...
    std::stringstream ss;
    ss << this->getName() << "_vdbLevel_" << level;
    grid->setName( ss.str() );
    return grid;
}
...
prolongateActiveVoxels(size_t destlevel, size_t grainSize)
{
    ...
    //GGG//
    // Since grids have cell-centered transform,
    // offset by 25% to be vertex-centered
    typename Grid<TreeType>::Ptr grid = this->grid(destlevel+1);
    math::Transform::Ptr xform = grid->transformPtr()->copy();
    Vec3d offset(-0.25, -0.25, -0.25);
    xform->preTranslate(offset);
    typename Grid<TreeType>::Ptr grid0 = Grid<TreeType>::create();
    grid0->setTransform(xform);
    util::NullInterrupter interrupter;
    // Resample using trilinear interpolation.
    doResampleToMatch<BoxSampler>(*grid, *grid0, interrupter);
    CookOp<ProlongateOp> tmp( grid0->tree(), fineTree, grainSize );
    //GGG//
...
bottomUpProlongate()
{
    ...
    for (int n=this->coarsestLevel()-1; n>=0; --n) {
        const TreeType &coarseTree = *mTrees[n+1];
        mTrees[n] = TreePtr(new TreeType( coarseTree.background() ) );// empty tree
        TreeType &fineTree = *mTrees[n];
        MaskFineTreeOp tmp(coarseTree, fineTree, 128);
        this->prolongateActiveVoxels(n, 64);
        if ( isLevelSet ) {
            tools::signedFloodFill( fineTree );
            tools::pruneLevelSet( fineTree );//only creates inactive tiles
        }
...
topDownRestrict(bool useInjection)
{
    ...
    for (size_t n=1; n<mTrees.size(); ++n) {
        ...
        // Since grids have cell-centered transform,
        // offset by 25% to be vertex-centered
        typename Grid<TreeType>::Ptr grid = this->grid(n);
        math::Transform::Ptr xform = grid->transformPtr()->copy();
        Vec3d offset(0.25, 0.25, 0.25);
        xform->preTranslate(offset);
        typename Grid<TreeType>::Ptr grid0 = Grid<TreeType>::create();
        ...
        // Resample using trilinear interpolation.
        doResampleToMatch<BoxSampler>(*grid, *grid0, interrupter);
        mTrees[n] = grid0->treePtr();// steal tree from input grid
        //GGG//
    }// loop over grid levels
}

template<typename TreeType>
struct MultiResGrid<TreeType>::MaskFineTreeOp
{
    ...
    MaskFineTreeOp(const TreeType& coarseTree, TreeType& fineTree, size_t grainSize = 1)
        : mPool(new PoolType( fineTree ) )// empty fine tree acts as examplar
    {
        assert( fineTree.empty() );

        // Create Mask of restruction performed on fineTree
        MaskT mask(coarseTree, false, true, TopologyCopy() );

        //tools::erodeActiveValues(mask, 1, NN_FACE_EDGE_VERTEX);
        tools::erodeActiveValues(mask, 1);
        ...
        // Prolongation using thread-local storage of coarse tree masks
        ManagerT leafs( mask );
        ...
        for (IterT it=mPool->begin(); it!=mPool->end(); ++it) fineTree.topologyUnion( *it );
        delete mPool;
    }
    void operator()(const RangeT& range) const
    {
        Accessor fineAcc( mPool->local() );// disabled registration
        for (typename RangeT::Iterator leafIter = range.begin(); leafIter; ++leafIter) {
            for (VoxelIterT voxelIter = leafIter->cbeginValueOn(); voxelIter; ++voxelIter) {
                ...
                ijk <<= 1;
                fineAcc.setValueOn( ijk );
                fineAcc.setValueOn( ijk.offsetBy(1,0,0) );
                fineAcc.setValueOn( ijk.offsetBy(0,1,0) );
                fineAcc.setValueOn( ijk.offsetBy(0,0,1) );
                fineAcc.setValueOn( ijk.offsetBy(1,1,0) );
                fineAcc.setValueOn( ijk.offsetBy(1,0,1) );
                fineAcc.setValueOn( ijk.offsetBy(0,1,1) );
                fineAcc.setValueOn( ijk.offsetBy(1,1,1) );
