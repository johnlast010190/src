...
#include "openvdb/tools/Prune.h" // for pruneInactive and pruneLevelSet
#include "openvdb/tools/SignedFloodFill.h" // for signedFloodFillWithValues
#include "openvdb/tools/Morphology.h" //for dilation and erosion //GGG
...
enum MeshToVolumeFlags {
    ...
    /// Compute signed distance field only for a part of the polygonal
    /// mesh inside a maskTree
    PARTIAL_SIGNED_DISTANCE_FIELD = 0x10 //16 in hexadecimal
};
...
meshToVolume(
  ...
  typename GridType::template ValueConverter<bool>::Type  * maskGrid = nullptr,
  typename GridType::template ValueConverter<Vec4i>::Type * polygonListGrid = nullptr,
  typename GridType::template ValueConverter<Vec3d>::Type * displacementGrid = nullptr);
...
public:
    ...
    using Vec4ITreeType = typename TreeType::template ValueConverter<Vec4i>::Type;
    using Vec3dTreeType = typename TreeType::template ValueConverter<Vec3d>::Type;

    using LeafNodeType = typename TreeType::LeafNodeType;
    using Int32LeafNodeType = typename Int32TreeType::LeafNodeType;
    using Vec4ILeafNodeType = typename Vec4ITreeType::LeafNodeType;
    using Vec3dLeafNodeType = typename Vec3dTreeType::LeafNodeType;

    CombineLeafNodes(TreeType& lhsDistTree, Int32TreeType& lhsIdxTree,
            Vec4ITreeType& lhsIdxlistTree, Vec3dTreeType& lhsDisplTree,
            LeafNodeType ** rhsDistNodes, Int32LeafNodeType ** rhsIdxNodes,
            Vec4ILeafNodeType ** rhsIdxlistNodes, Vec3dLeafNodeType ** rhsDisplNodes)
        : mDistTree(&lhsDistTree)
        , mIdxTree(&lhsIdxTree)
        , mIdxlistTree(&lhsIdxlistTree)
        , mDisplTree(&lhsDisplTree)
        , mRhsDistNodes(rhsDistNodes)
        , mRhsIdxNodes(rhsIdxNodes)
        , mRhsIdxlistNodes(rhsIdxlistNodes)
        , mRhsDisplNodes(rhsDisplNodes)
    {
    }
    ...
    void operator()(const tbb::blocked_range<size_t>& range) const {
        ...
        tree::ValueAccessor<Vec4ITreeType> idxlistAcc(*mIdxlistTree);
        tree::ValueAccessor<Vec3dTreeType> displAcc(*mDisplTree);

        using DistValueType = typename LeafNodeType::ValueType;
        using IndexValueType = typename Int32LeafNodeType::ValueType;
        using IndexlistValueType = typename Vec4ILeafNodeType::ValueType;
        using DisplValueType = typename Vec3dLeafNodeType::ValueType;
        ...
        for (size_t n = range.begin(), N = range.end(); n < N; ++n) {
            ...
            Vec4ILeafNodeType* lhsIdxlistNode = idxlistAcc.probeLeaf(origin);
            Vec3dLeafNodeType* lhsDisplNode = displAcc.probeLeaf(origin);

            DistValueType* lhsDistData = lhsDistNode->buffer().data();
            IndexValueType* lhsIdxData = lhsIdxNode->buffer().data();
            IndexlistValueType* lhsIdxlistData = lhsIdxlistNode->buffer().data();
            DisplValueType* lhsDisplData = lhsDisplNode->buffer().data();

            const DistValueType* rhsDistData = mRhsDistNodes[n]->buffer().data();
            const IndexValueType* rhsIdxData = mRhsIdxNodes[n]->buffer().data();
            const IndexlistValueType* rhsIdxlistData = mRhsIdxlistNodes[n]->buffer().data();
            const DisplValueType* rhsDisplData = mRhsDisplNodes[n]->buffer().data();
            ...
            for (Index32 offset = 0; offset < LeafNodeType::SIZE; ++offset) {
                ...
                if (rhsIdxData[offset] != Int32(util::INVALID_IDX)) {
                    ...
                    if (rhsValue < lhsValue) {
                        ...
                        lhsDisplNode->setValueOn(offset, rhsDisplData[offset]);
                        //TODO if lhsIdxlist.size() < 4 append rhs elements
                        // currently I'm just copying them over
                        lhsIdxlistNode->setValueOn(offset, rhsIdxlistData[offset]);
            ...
            delete mRhsIdxlistNodes[n];
            delete mRhsDisplNodes[n];
        }
    }
...
private:
    ...
    Vec4ITreeType * const mIdxlistTree;
    Vec3dTreeType * const mDisplTree;

    LeafNodeType ** const mRhsDistNodes;
    Int32LeafNodeType ** const mRhsIdxNodes;
    Vec4ILeafNodeType ** const mRhsIdxlistNodes;
    Vec3dLeafNodeType ** const mRhsDisplNodes;
}; // class CombineLeafNodes
    ...
    void operator()(const tbb::blocked_range<size_t>& range) const {
        ...
        for (size_t n = range.begin(), N = range.end(); n < N; ++n) {
            ...
            for (it = node.cbeginValueOn(); it; ++it) {
                ...
                ValueType& dist = data[pos];
                if (dist < 0.0 || dist > mIntersectDist) continue;
                //for partialSDF do not touch intersecting voxels
                if (mIntersectDist > 0.75 && dist < 0.75) continue;
...
template<typename TreeType>
struct FlipExternalVoxelSign
{
    using ValueType = typename TreeType::ValueType;
    using LeafNodeType = typename TreeType::LeafNodeType;
    using BoolTreeType = typename TreeType::template ValueConverter<bool>::Type;

    FlipExternalVoxelSign(
        std::vector<LeafNodeType*>& distNodes,
        BoolTreeType& signMaskTree)
        : mDistNodes(distNodes.empty() ? nullptr : &distNodes[0])
        , mSignMaskTree(&signMaskTree)
    {
    }
    ...
    void operator()(const tbb::blocked_range<size_t>& range) const {
        ...
        tree::ValueAccessor<const BoolTreeType> signAcc(*mSignMaskTree);
        ...
        for (size_t n = range.begin(), N = range.end(); n < N; ++n) {
            ...
            for (it = node.cbeginValueOn(); it; ++it) {
                ...
                ValueType& dist = data[pos];
                if (dist < 0.75*9) continue;

                ijk = node.offsetToGlobalCoord(pos);

                if (!signAcc.getValue(ijk)) {
                    dist = -dist;
                }
            } // active voxel loop
        } // leaf node loop
...
private:
    ...
    LeafNodeType         ** const mDistNodes;
    BoolTreeType    const * const mSignMaskTree;
}; // FlipExternalVoxelSign


template<typename TreeType>
struct ComputeMaskVoxelSign
{
    using ValueType = typename TreeType::ValueType;
    using LeafNodeType = typename TreeType::LeafNodeType;
    using BoolTreeType = typename TreeType::template ValueConverter<bool>::Type;
    ...
    void operator()(const tbb::blocked_range<size_t>& range) const {
        ...
        typename BoolLeafNodeType::ValueOnCIter it;

        for (size_t n = range.begin(), N = range.end(); n < N; ++n) {

            BoolLeafNodeType& node = *mMaskNodes[n];
            ...
            for (it = node.cbeginValueOn(); it; ++it) {
                ...
                for (Int32 i = 0; i < 26; ++i) {
                    ...
                    if (distAcc.probeValue(nijk, nval)) {
                        if (nval < 0.0) {
                            distAcc.setValueOn(ijk, -mInteriorBandWidth);
                        } else {
                            distAcc.setValueOn(ijk, mExteriorBandWidth);
                        }
                        break;
                    }
                }
...
private:
    ...
    const ValueType mExteriorBandWidth, mInteriorBandWidth;
}; // ComputeMaskVoxelSign
...
template<typename DistTreeType, typename IndexTreeType, typename IdxlistTreeType, typename DisplTreeType>
inline void
combineData(DistTreeType& lhsDist, IndexTreeType& lhsIdx, IdxlistTreeType& lhsIdxlist, DisplTreeType& lhsDispl,
            DistTreeType& rhsDist, IndexTreeType& rhsIdx, IdxlistTreeType& rhsIdxlist, DisplTreeType& rhsDispl)
{
    using DistLeafNodeType    = typename DistTreeType::LeafNodeType;
    using IndexLeafNodeType   = typename IndexTreeType::LeafNodeType;
    using IdxlistLeafNodeType = typename IdxlistTreeType::LeafNodeType;
    using DisplLeafNodeType   = typename DisplTreeType::LeafNodeType;

    std::vector<DistLeafNodeType*>    overlappingDistNodes;
    std::vector<IndexLeafNodeType*>   overlappingIdxNodes;
    std::vector<IdxlistLeafNodeType*> overlappingIdxlistNodes;
    std::vector<DisplLeafNodeType*>   overlappingDisplNodes;
    ...
    tasks.run(StealUniqueLeafNodes<IdxlistTreeType>(lhsIdxlist, rhsIdxlist, overlappingIdxlistNodes));
    tasks.run(StealUniqueLeafNodes<DisplTreeType>  (lhsDispl, rhsDispl, overlappingDisplNodes));
    tasks.wait();

    // Combine overlapping leaf nodes (distance)
    if (!overlappingDistNodes.empty() && !overlappingIdxNodes.empty()) {
        tbb::parallel_for(tbb::blocked_range<size_t>(0, overlappingDistNodes.size()),
            CombineLeafNodes<DistTreeType>(lhsDist, lhsIdx, lhsIdxlist, lhsDispl,
                &overlappingDistNodes[0], &overlappingIdxNodes[0],
                &overlappingIdxlistNodes[0], &overlappingDisplNodes[0]));
    }
}
...
struct VoxelizationData {
    ...
    using BoolTreeAcc  = tree::ValueAccessor<BoolTreeType>;
    using Vec4ITreeAcc = tree::ValueAccessor<Vec4ITreeType>;
    using Vec3dTreeAcc = tree::ValueAccessor<Vec3dTreeType>;
    ...
    VoxelizationData()
        ...
        , idxlistTree(Vec4i(Int32(util::INVALID_IDX)))
        , idxlistAcc(idxlistTree)
        , displTree(Vec3d(0))
        , displAcc(displTree)
        , maskTree(false)
        , maskAcc(maskTree)
    ...
    Vec4ITreeType   idxlistTree;
    Vec4ITreeAcc    idxlistAcc;

    Vec3dTreeType   displTree;
    Vec3dTreeAcc    displAcc;

    BoolTreeType    maskTree;
    BoolTreeAcc     maskAcc;

    void setMaskTree(const BoolTreeType& mask) {
        maskTree.topologyUnion(mask);
    }
...
public:
    ...
    VoxelizePolygons(DataTable& dataTable,
        ...
        BoolTree* maskTreePtr = nullptr)
        : mDataTable(&dataTable)
        , mMesh(&mesh)
        , mInterrupter(interrupter)
        , mMaskTreePtr(maskTreePtr)
    {
    }
    ...
    void operator()(const tbb::blocked_range<size_t>& range) const {
        ...
        if (mMaskTreePtr) dataPtr->setMaskTree(*mMaskTreePtr);
...
private:
    ...
    struct SubTask
    {
        ...
        void operator()() const
        {
            if (mSubdivisionCount <= 0 || mPolygonCount >= POLYGON_LIMIT) {
                ...
                voxelizeTriangle(mPrim, *dataPtr, mInterrupter, mMaskTreePtr);

            } else if (!(mInterrupter && mInterrupter->wasInterrupted())) {
                spawnTasks(mPrim, *mLocalDataTable, mSubdivisionCount, mPolygonCount, mInterrupter, mMaskTreePtr);
            }
        }
    ...
    inline static int evalSubdivisionCountMask(const Triangle& prim)
    {
        const double ax = prim.a[0], bx = prim.b[0], cx = prim.c[0];
        const double dx = std::max(ax, std::max(bx, cx)) - std::min(ax, std::min(bx, cx));
        ...
        return int(std::max(dx, std::max(dy, dz)) / 4.);
    }
    ...
    void evalTriangle(const Triangle& prim, VoxelizationDataType& data) const
    {
        ...
        // force triangle subdivision in case of sliver triangle where
        // the centre intersects the mask but the vertices don't
        int subdivisionCount;
        if (mMaskTreePtr) {
            subdivisionCount = evalSubdivisionCountMask(prim);
        } else {
            subdivisionCount =
        ...
        if (subdivisionCount <= 0) {
            voxelizeTriangle(prim, data, mInterrupter, mMaskTreePtr);
        } else {
            spawnTasks(prim, *mDataTable, subdivisionCount, polygonCount, mInterrupter, mMaskTreePtr);
        }
    }
    ...
    static void spawnTasks(
        ...
        BoolTree* const maskTreePtr)
    {
        subdivisionCount -= 1;
        if (!maskTreePtr) polygonCount *= 4;
        ...
        prim.b = ab;
        prim.c = ac;
        if (maskTreePtr) subdivisionCount = evalSubdivisionCountMask(prim);
        tasks.run(SubTask(prim, dataTable, subdivisionCount, polygonCount, interrupter, maskTreePtr));
    ...
    static void voxelizeVertex(Coord& ijk, const Triangle& prim, const unsigned char& primId, VoxelizationDataType& data)
    {
        ...
        while (!coordList.empty()) {
            for (Int32 pass = 0; pass < 1048576 && !coordList.empty(); ++pass) {
                ...
                for (Int32 i = 0; i < 26; ++i) {
                    nijk = ijk + util::COORD_OFFSETS[i];
                    if (!data.maskAcc.isValueOn(nijk)) continue;
                    ...
                    //// loop through neighbours of neighbours
                    //// as we don't expandNarrowBand when mask is provided
                    //for (Int32 j = 0; j < 26; ++j) {
                    //    nnijk = nijk + util::COORD_OFFSETS[j];
                    //    if (!data.maskAcc.isValueOn(nnijk)) continue;
                    //    if (primId != data.primIdAcc.getValue(nnijk)) {
                    //        data.primIdAcc.setValueOnly(nnijk, primId);
                    //        if(updateDistance(nnijk, prim, data)) coordList.push_back(nnijk);
                    //    }
                    //}
                }
            }
    ...
    static void voxelizeTriangle(const Triangle& prim, VoxelizationDataType& data, Interrupter* const interrupter, BoolTree* const maskTreePtr)
    {
        std::deque<Coord> coordList;
        Coord ijk, nijk, nnijk;

        ijk = Coord::floor(prim.a);

        if (maskTreePtr)
        {
            Coord ijkb = Coord::floor(prim.b);
            Coord ijkc = Coord::floor(prim.c);

            ////debug: triangle vertices inside mask
            //if (data.maskAcc.isValueOn(ijk)) updateDistance(ijk, prim, data);
            //if (data.maskAcc.isValueOn(ijkb)) updateDistance(ijkb, prim, data);
            //if (data.maskAcc.isValueOn(ijkc)) updateDistance(ijkc, prim, data);
            //return;

            ////debug all mask
            //for (auto iter = data.maskTree.cbeginValueOn(); iter; ++iter)
            //{
            //    const Coord& xyz = iter.getCoord();
            //    updateDistance(xyz, prim, data);
            //}
            //return;

            unsigned char primId = data.getNewPrimId();

            // start from each vertex in case of a big triangle
            // with vertices in the mask but not the centre
            if (data.maskAcc.isValueOn(ijk)) {
                voxelizeVertex(ijk, prim, primId, data);
            }
            /*else*/ if (data.maskAcc.isValueOn(ijkb)) {
                voxelizeVertex(ijkb, prim, primId, data);
            }
            /*else*/ if (data.maskAcc.isValueOn(ijkc)) {
                voxelizeVertex(ijkc, prim, primId, data);
            }
    ...
    static bool updateDistance(const Coord& ijk, const Triangle& prim, VoxelizationDataType& data)
    {
        ...
        const Vec3d displ = (voxelCenter -
            closestPointOnTriangleToPoint(prim.a, prim.c, prim.b, voxelCenter, uvw));

        const ValueType dist = ValueType(displ.lengthSqr());
        ...
        // update displacement and list of triangle ID
        // only for intersecting voxels (and neighbours)
        //if (dist < 1.5)
        {
            // update list of triangle ID
            Vec4i IDs = data.idxlistAcc.getValue(ijk);

            if (dist < oldDist) {

                data.displAcc.setValue(ijk, displ);
                
                // move IDs
                for (Int32 i = 2; i >=0; --i) {
                    if (IDs[i] != Int32(util::INVALID_IDX)) {
                        IDs[i+1] = IDs[i];
                    }
                }
                IDs[0] = prim.index;
                data.idxlistAcc.setValue(ijk, IDs);
            } else {
                for (Int32 i = 0; i < 4; ++i) {
                    if (IDs[i] == Int32(util::INVALID_IDX)) {
                        IDs[i] = prim.index;
...
meshToVolume(
  ...
  typename GridType::template ValueConverter<bool>::Type  * maskGrid,
  typename GridType::template ValueConverter<Vec4i>::Type * polygonListGrid,
  typename GridType::template ValueConverter<Vec3d>::Type * displacementGrid)
{
    ...
    using Vec4IGridType = typename GridType::template ValueConverter<Vec4i>::Type;
    using Vec4ITreeType = typename Vec4IGridType::TreeType;

    using Vec3dGridType = typename GridType::template ValueConverter<Vec3d>::Type;
    using Vec3dTreeType = typename Vec3dGridType::TreeType;

    using BoolGridType = typename GridType::template ValueConverter<bool>::Type;
    using BoolTreeType = typename BoolGridType::TreeType;
    using BoolLeafNodeType = typename BoolTreeType::LeafNodeType;
    ...
    const bool partialSignedDistanceField = (flags & PARTIAL_SIGNED_DISTANCE_FIELD);

    // initialize indexGrid
    ...
    // initialize idxlistGrid
    Vec4IGridType* idxlistGrid = nullptr;

    typename Vec4IGridType::Ptr temporaryIdxlistGrid;

    if (polygonListGrid) {
        idxlistGrid = polygonListGrid;
    } else {
        temporaryIdxlistGrid.reset(new Vec4IGridType(Vec4i(Int32(util::INVALID_IDX))));
        idxlistGrid = temporaryIdxlistGrid.get();
    }

    idxlistGrid->newTree();
    idxlistGrid->setTransform(transform.copy());

    // initialize displGrid
    Vec3dGridType* displGrid = nullptr;

    typename Vec3dGridType::Ptr temporaryDisplGrid;

    if (displacementGrid) {
        displGrid = displacementGrid;
    } else {
        temporaryDisplGrid.reset(new Vec3dGridType(Vec3d(0)));
        displGrid = temporaryDisplGrid.get();
    }

    displGrid->newTree();
    displGrid->setTransform(transform.copy());
    ...
    Vec4ITreeType& idxlistTree = idxlistGrid->tree();
    Vec3dTreeType& displTree   = displGrid->tree();
...

    {
        ...
        if (partialSignedDistanceField || maskGrid) {
            if (!maskGrid) {
                OPENVDB_THROW(TypeError, "I need a maskGrid for partial Signed Distance Field");
            }
            tbb::parallel_for(polygonRange, Voxelizer(data, mesh, &interrupter, &maskGrid->tree()));
        } else {
        ...
        for (typename DataTable::iterator i = data.begin(); i != data.end(); ++i) {
            ...
            mesh_to_volume_internal::combineData(
                distTree,          indexTree,          idxlistTree,          displTree,
                dataItem.distTree, dataItem.indexTree, dataItem.idxlistTree, dataItem.displTree);
        }
    }
    ...
    if (computeSignedDistanceField) {
        ...
        if (partialSignedDistanceField) {
            // Flip sign of external voxels based on sign of coarse maskGrid
            using ExternalSignOp =
                mesh_to_volume_internal::FlipExternalVoxelSign<TreeType>;

            tbb::parallel_for(nodeRange, ExternalSignOp(nodes, maskGrid->tree()));

            // Propagate sign up to intersecting voxels
            tbb::parallel_for(nodeRange, SignOp(nodes, distTree, indexTree, mesh, 0.75*9));
            // do double pass for voxels missed by previous SignOp
            tbb::parallel_for(nodeRange, SignOp(nodes, distTree, indexTree, mesh, 0.75*9));
        } else {
    ...
    // Fill the mask with signed values
    if (partialSignedDistanceField) {
        ...
        while (true) {
            ...
            maskGrid->topologyDifference(*distGrid);

            const size_t maskNodeCount = maskGrid->tree().leafCount();
            //if (maskNodeCount == 0) break;
            if (maskGrid->activeVoxelCount() == 0) break;

            nodes.clear();
            nodes.reserve(maskNodeCount);
            maskGrid->tree().getNodes(nodes);

            const tbb::blocked_range<size_t> nodeRange(0, nodes.size());

            using MaskSignOp =
                mesh_to_volume_internal::ComputeMaskVoxelSign<TreeType>;

            tbb::parallel_for(nodeRange, MaskSignOp(nodes, distTree, exteriorWidth, interiorWidth));
        }
    ...
    if (!polygonListGrid)  idxlistGrid->clear();
    if (!displacementGrid) displGrid->clear();
    ...
    if (computeSignedDistanceField && renormalizeValues && !partialSignedDistanceField) {
    ...
    // trim extra voxels exceeding maskGrid generated during expandNarrowBand
    if (maskGrid) {
        distGrid->topologyIntersection(*maskGrid);
    }
  ...
  typename GridType::template ValueConverter<Vec3d>::Type * displacementGrid)
{
    ...
    return meshToVolume<GridType>(nullInterrupter, mesh, transform,
        exteriorBandWidth, interiorBandWidth, flags, polygonIndexGrid, maskGrid,
        polygonListGrid, displacementGrid);
}
