...
)
{
    ...
    (
        pp.index(),
        facei,
        (coupleMass_)? mass : Zero,               // mass
        (coupleMomentum_)? mass*Ut : Zero,        // tangential momentum
        (coupleMomentum_)? mass*mag(Un) : Zero,    // impingement pressure
        (coupleEnergy_)? mass*p.hs() : Zero     // energy
    );
    ...
    coupleEnergy_(true)
{
    ...
    coupleMomentum_=
            this->coeffDict().lookupOrDefault("coupleMomentum",true);
    if (!coupleMomentum_)
    {
        Info<< "    Decoupling parcel-film momentum transfer to prevent"
            << " film convergence issues."<< endl;
    }

    coupleMass_=
            this->coeffDict().lookupOrDefault("coupleMass",true);
    if (!coupleMass_)
    {
        Info<< "    Decoupling parcel-film mass transfer to prevent"
            << " film convergence issues."<< endl;
    }

    coupleEnergy_=
            this->coeffDict().lookupOrDefault("coupleEnergy",true);
    if (!coupleEnergy_)
    {
        Info<< "    Decoupling parcel-film energy transfer to prevent"
            << " film convergence issues."<< endl;
    }
}
...
:
    ...
    coupleMomentum_(sfm.coupleMomentum_),
    coupleMass_(sfm.coupleMass_),
    coupleEnergy_(sfm.coupleEnergy_)
{}
